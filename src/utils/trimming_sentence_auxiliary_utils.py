...
def split_by_LLM(transcript, translate, model="gpt-3.5-turbo-1106"):
    prompt = f"""请将以下两个句子分割成多个独特的部分。句子应根据自然的语法或语义断点（如逗号、句号、连接词）进行分割。如果两个语言版本的分割部分数量不一致，请调整较长的部分以匹配较短的部分。这意味着，如果一个语言版本的句子被分割成更多的部分，您需要将这些额外的部分合并，以确保每对 `sentence1` 和 `sentence2` 在数量和语义上相匹配并且在 `check` 字段记录匹配结论，从而避免重复或空缺。在这个任务中，目标是保持原文的意义和结构，同时在适当的位置进行分割，以确保两种语言之间的对应关系清晰明确且意义一致。
